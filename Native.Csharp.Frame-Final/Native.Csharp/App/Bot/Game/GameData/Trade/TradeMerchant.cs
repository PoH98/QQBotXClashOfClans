...
namespace Native.Csharp.App.Bot.GameData.Trade
{
    ...
    {
        private List<TradeItem> tradeItems = new List<TradeItem>();
        public void Dispose()
        {
            SaveData();
        }
        ...
        internal string AddItem(Element element, long price, int amount, GameAPI owner)
        {
            ...
            newitem.UniqueId = Regex.Replace(Convert.ToBase64String(Guid.NewGuid().ToByteArray()), "[/+=]", "").Substring(0, 8).ToUpper();
            newitem.Item = element;
            newitem.Owner = owner.Member;
            newitem.Price = price;
        ...
        internal string PurchaseItem(GameAPI buyer, string guid)
        {
            ...
            else
            {
                ...
                using (GameAPI owner = new GameAPI(item.Owner.Member.GroupId, item.Owner.Member.QQId))
                {
